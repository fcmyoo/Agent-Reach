...
이 프로젝트는 자유분방하게 개발되었습니다 🎸 다소 거친 부분이 있을 수 있지만 양해 부탁드립니다! 버그를 발견하면 주저하지 말고 [Issue](https://github.com/fcmyoo/Agent-Reach/issues)를 열어주세요. 최대한 빨리 수정하겠습니다.

**새 채널을 원하시나요?** Issue를 열어 요청하거나, 직접 PR을 제출하세요.

**로컬에 추가하고 싶나요?** 에이전트가 저장소를 복제하고 수정하게 하세요 — 각 채널은 단일 독립 파일이므로 추가하기 쉽습니다.

[PR](https://github.com/fcmyoo/Agent-Reach/pulls)은 언제든 환영합니다!
...
네! Agent Reach는 설치 + 설정 도구입니다. Shell 명령을 실행할 수 있는 모든 AI 코딩 에이전트가 사용할 수 있습니다 — Claude Code, Cursor, Windsurf, OpenClaw, Codex 등. `pip install https://github.com/fcmyoo/Agent-Reach/archive/main.zip` 실행 후 먼저 `agent-reach install`로 읽기 전용 검사를 하고, 시스템 변경을 명시적으로 승인한 경우에만 `agent-reach install --system`을 실행합니다. PyPI의 동명 패키지는 다른 프로젝트입니다.
</details>
...
> 버그 보고 및 기능 요청은 [GitHub Issues](https://github.com/fcmyoo/Agent-Reach/issues)를 이용해주세요 — 추적이 더 수월합니다.
